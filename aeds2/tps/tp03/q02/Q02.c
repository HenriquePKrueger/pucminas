...
void insercao_parcial(Restaurante** restaurantes, int n, int k){
	for(int i = 1; i < n; i++){
		Restaurante* tmp = restaurantes[i];
		movimentacoes++;
		int j = (i < k) ? i - 1 : k - 1;

		while((j >= 0) && strcmp(restaurantes[j]->cidade, tmp->cidade) > 0){
			comparacoes++;
...
void gerar_log(double tempo){
	FILE* log = fopen("899683_insercao_parcial.txt", "w");
...
int main(){
	...
	insercao_parcial(restaurantesBuscados.restaurante, restaurantesBuscados.tamanho, k);
	clock_t fim = clock();
