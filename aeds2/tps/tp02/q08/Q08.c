...
//Função de particionar para o  quicksort
int particionar(Colecao_Restaurantes* c, int esq, int dir){
	Restaurante* pivo = c->restaurante[dir];
	int i = (esq - 1);
	...
	for(int j = esq; j <= dir - 1; j++){
		...
		if(c->restaurante[j]->avaliacao < pivo->avaliacao){
			menor = true;
		}
		else if(c->restaurante[j]->avaliacao == pivo->avaliacao){
			comparacoes++;
			if(strcmp(c->restaurante[j]->nome, pivo->nome) < 0){
				menor = true;
			}
		}
		...
		if(menor == true){
			i++;
			swap(&c->restaurante[i], &c->restaurante[j]);
		}
	}
	swap(&c->restaurante[i+1], &c->restaurante[dir]);
	return (i + 1);
}

//Algorítmo quicksort
void quicksort(Colecao_Restaurantes* c, int esq, int dir) {
	if(esq < dir){
		int pi = particionar(c, esq, dir);
		quicksort(c, esq, pi-1);
		quicksort(c, pi+1, dir);
	}
}
...
int main(){
	...
	quicksort(&restaurantesBuscados, 0, restaurantesBuscados.tamanho - 1);
	...
	for(int i = 0; i < restaurantesBuscados.tamanho; i++){//Printa na tela os resultados
		formatar_restaurante(restaurantesBuscados.restaurante[i], buffer);
		printf("%s\n", buffer);
	}
	
	//Gerar o arquivo de log
	FILE* log = fopen("899683_quicksort.txt", "w");
	if(log != NULL){
