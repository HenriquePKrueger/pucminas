...
Hora parse_hora(char* s){
	...
	sscanf(s, "%d:%d", &h.hora, &h.minuto);//Usar "." por que está apontando diretamente para a struct
	return h;
}

void formatar_hora(Hora* hora, char* buffer){
	sprintf(buffer, "%02d:%02d", hora->hora, hora->minuto);//Usar "->" por que, nesse caso, trata-se de um ponteiro
}
...
void inserir(struct PilhaSequencial* p, Restaurante* restaurante){
	p->restaurantes[p->n] = restaurante;
	p->n++;
}

Restaurante* remover(struct PilhaSequencial* p){
	return p->restaurantes[--p->n];
}

int main(){
	Colecao_Restaurantes* c = ler_csv();
	struct PilhaSequencial pilha;//Guarda os dados
	pilha.restaurantes = (Restaurante**) malloc(c->tamanho * sizeof(Restaurante*));
	pilha.n = 0;
	...
	//Parte 1
	while(id != -1){//Lê os ids até o "-1"
		for(int i = 0; i < c->tamanho; i++){
			if(c->restaurante[i]->id == id){
				 inserir(&pilha, c->restaurante[i]);
			}
		}
		scanf("%d", &id);	
	}
	...
	for(int i = pilha.n - 1; i >= 0; i--){
		formatar_restaurante(pilha.restaurantes[i], buffer);
		printf("%s\n", buffer);
	}
