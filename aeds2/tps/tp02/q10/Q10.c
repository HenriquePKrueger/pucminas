...
//Algorítmo countingsort
void countingSort(Colecao_Restaurantes* c){
	if(c->tamanho <= 0) return;
	
	int max = c->restaurante[0]->capacidade;
	for(int i = 1; i < c->tamanho; i++){
		comparacoes++;
		if(c->restaurante[i]->capacidade > max){
			max = c->restaurante[i]->capacidade;
		}
	}
	
	int* count = (int*) calloc(max + 1, sizeof(int));
	Restaurante** ordenado = (Restaurante**) malloc(c->tamanho * sizeof(Restaurante*));
	
	for(int i = 0; i < c->tamanho; i++){
		count[c->restaurante[i]->capacidade]++;
	}
	...
	for(int i = c->tamanho-1; i>=0;i--){
		ordenado[count[c->restaurante[i]->capacidade]-1] = c->restaurante[i];
		count[c->restaurante[i]->capacidade]--;
		movimentacoes++;
	}
	
	for(int i = 0; i < c->tamanho; i++){
		c->restaurante[i] = ordenado[i];
		movimentacoes++;
	}
...
int main(){
	...
	countingSort(&restaurantesBuscados);
	...
	FILE* log = fopen("899683_countingsort.txt", "w");
	if(log != NULL){
